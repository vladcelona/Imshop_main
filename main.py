    ...
    while True:
        ...
        if 200 >= code.status_code <= 299:
            ...
            for index in range(len(all_replies_info)):
                ...
                all_replies_dict[index][columns[0]] = all_replies_info[index].find('h3').text.strip().replace('\n', '')
                all_replies_dict[index][columns[1]] = \
                    all_replies_info[index].find('span', {'class': 'ui-star-rating__rating visuallyhidden'}).text.replace(' of ', '/')
                all_replies_dict[index][columns[2]] = \
                    all_replies_info[index].find_all('div', {'class': 'review-metadata__item-value'})[1].text.strip()\
                        .replace('\n', '')
